...
const MANGA_RECOMMENDATIONS = [
  {
    ...
    author: "Gege Akutami",
    coverUrl: "https://cdn.myanimelist.net/images/manga/3/210333.jpg",
    subjects: ["manga", "action", "supernatural"]
  },
  {
    title: "Vinland Saga",
    author: "Makoto Yukimura",
    coverUrl: "https://cdn.myanimelist.net/images/manga/2/188041.jpg",
...
export default function Home() {
  ...
  // Real-time search using Open Library (No LLM hallucinations)
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await fetch(`https://openlibrary.org/search.json?q=${encodeURIComponent(query)}&limit=6&fields=title,author_name,cover_i,subject`);
  ...
  const handleSelectBook = async (book) => {
    ...
    try {
      const res = await fetch('/api/match-music', {
        method: 'POST',
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white p-6 max-w-4xl mx-auto flex flex-col items-center">
      ...
      <div className="relative w-full max-w-xl mb-8">
        ...
        {suggestions.length > 0 && (
          <ul className="absolute z-50 w-full mt-1 bg-gray-900 border border-gray-700 rounded-lg shadow-xl overflow-hidden">
      ...
      {/* Selected Book & Music Tracks Display */}
      {loading ? (
      ...
      {/* Fixed Manga Recommendations Section */}
      <div className="w-full mt-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {MANGA_RECOMMENDATIONS.map((manga, idx) => (
            ...
            >
              <div className="w-full h-44 bg-gray-800 rounded overflow-hidden mb-2 relative flex items-center justify-center">
                <img
                  src={manga.coverUrl}
                  alt={manga.title}
                  className="w-full h-full object-cover"
                  onError={(e) => {
                    e.target.style.display = 'none'; // Fallback if link breaks
                  }}
                />
                <span className="absolute text-xs text-gray-500 -z-10">{manga.title}</span>
              </div>
